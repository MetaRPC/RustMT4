# MT4 Return Codes (RetCodes) Reference - Python

## 🛡️ What is RetCode?

**RetCode** (Return Code) is a status code that MT4 terminal returns after executing a trading operation. The code indicates success or the reason for failure.
...
- **Unified with MT4 terminal** - returned directly from the trading server
- **Defined in protobuf** - returned as `uint32` field `returned_code` in trading result structures
- **Available as Python constants** - defined in `package/MetaRpcMT4/helpers/errors.py`
...
```python
from MetaRpcMT4.helpers.errors import (
    ApiError,
    TRADE_RETCODE_DONE,
    get_retcode_message,
    is_retcode_success
...
```python
from MetaRpcMT4.helpers.errors import TRADE_RETCODE_DONE, is_retcode_success, get_retcode_message
...
```python
from MetaRpcMT4.helpers.errors import ApiError, TRADE_RETCODE_DONE, TRADE_RETCODE_POSITION_CLOSED
...
**Helper function:**
```python
from MetaRpcMT4.helpers.errors import is_retcode_requote
...
**Helper function:**
```python
from MetaRpcMT4.helpers.errors import is_retcode_retryable
import asyncio
...
## Centralized Error Handler (`package/MetaRpcMT4/helpers/errors.py`)

**PyMT4 provides a centralized error handling module** that simplifies working with RetCodes.

Instead of manually checking return codes and writing your own error messages, you can use the built-in helper functions from `package/MetaRpcMT4/helpers/errors.py`:
...
The `package/MetaRpcMT4/helpers/errors.py` module provides helper functions for working with RetCodes:
...
```python
from MetaRpcMT4.helpers.errors import is_retcode_success, TRADE_RETCODE_DONE
...
```python
from MetaRpcMT4.helpers.errors import is_retcode_requote, TRADE_RETCODE_REQUOTE, TRADE_RETCODE_PRICE_CHANGED
...
```python
from MetaRpcMT4.helpers.errors import get_retcode_message, TRADE_RETCODE_DONE
...
```python
from MetaRpcMT4.helpers.errors import check_retcode, TRADE_RETCODE_DONE
...
```python
from MetaRpcMT4.helpers.errors import check_retcode
...
All RetCode constants are defined in `package/MetaRpcMT4/helpers/errors.py`:
