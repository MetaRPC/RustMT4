...
> Project-specific terms and concepts. This glossary covers PyMT4 architecture, components, and trading automation terminology used throughout the codebase.
...
Core design pattern of PyMT4 with three abstraction levels:

- **Tier 1 (MT4Account):** Low-level gRPC communication with MT4 terminal
- **Tier 2 (MT4Service):** Wrapper methods with dataclasses and Python types
- **Tier 3 (MT4Sugar):** High-level convenience methods with ready patterns
...
Base layer providing direct access to MT4 terminal via gRPC protocol.

**Key characteristics:**

- Raw gRPC calls to MT4 terminal
...
- **42 async methods**, covering all MT4 operations
...
- Primary purpose: Architectural middleware layer for MT4Sugar
...
**When to use:** Building custom strategies that need more control than Sugar but less complexity than MT4Account.
...
- Context manager support (`async with MT4Sugar(service) as sugar`)
...
**All methods in PyMT4 are async:**
```python
import asyncio
from MetaRpcMT4 import MT4Account
from MetaRpcMT4 import mt5_term_api_account_information_pb2 as account_info_pb2
...
**MT4Service uses dataclasses for clean data structures:**
```python
...
**MT4Sugar uses properties for instant access:**
```python
...
async with MT4Sugar(service) as sugar:
...
`account.channel` is the gRPC connection to MT4 terminal. You MUST call `await account.channel.close()` to:
...
**Where:** MT4 terminal itself handles normalization (both MT4Service and MT4Sugar benefit)
...
```python
from MetaRpcMT4 import mt5_term_api_market_info_pb2 as market_info_pb2
...
**In PyMT4:** All APIs use **points** for consistency.
...
- MT4Account tier sends gRPC requests to MT4 terminal
...
- All MT4Account methods use protobuf Request/Response
...
# MT4Account level - already unpacks protobuf response internally
balance = await account.account_info_double(
    account_info_pb2.AccountInfoDoublePropertyType.ACCOUNT_BALANCE
)
...
# MT4Service level - same result, just different method name
...
**Both MT4Account and MT4Service return clean Python types (float, int, str), not protobuf objects.**
...
Core trading concepts that every trader must understand before working with MT4.
...
# NOTE: MT4Sugar doesn't have delete_order method
# To cancel pending orders, use MT4Service or MT4Account level methods
```
...
```powershell
$env:MT4_USER="591129415"
$env:MT4_PASSWORD="YourPassword"
```

**Set (Linux/Mac):**
```bash
export MT4_USER=591129415
export MT4_PASSWORD="YourPassword"
```
...
user = int(os.getenv("MT4_USER"))
password = os.getenv("MT4_PASSWORD")
server = os.getenv("MT4_SERVER")
grpc_host = os.getenv("MT4_GRPC_HOST")
```
...
**Note:** To delete pending orders, use MT4Service or MT4Account level methods.
...
from MetaRpcMT4 import mt5_term_api_account_helper_pb2 as account_helper_pb2
...
history = await account.order_history(
    from_dt=start_date,
    to_dt=end_date,
    sort_mode=account_helper_pb2.BMT4_SORT_BY_OPEN_TIME_ASC,
    page_number=0,
    items_per_page=100
)
...
PyMT4 examples and helpers are educational materials and API demonstrations, NOT production trading systems.
...
**Custom exceptions:**
```python
from MetaRpcMT4 import ConnectExceptionMT4, ApiExceptionMT4
...
try:
    ...
    # MT4Sugar raises RuntimeError if returned_code != 10009
...
**All PyMT4 code uses type hints:**
```python
...
**PyMT4 uses enums instead of magic numbers:**
```python
# MT4Sugar enums
